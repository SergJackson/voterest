...
public class VoteRestController {
    ...
    @PostMapping(path = "/{restId}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Vote> createWithLocation(@AuthenticationPrincipal AuthUser authUser, @PathVariable int restId, @Valid @RequestBody Vote vote) {
        int userId = authUser.id();
        Optional<Restaurant> restaurant = restaurantRepository.get(restId);
        log.info("create {} for user {}", vote, userId);
        //checkNew(vote);
        checkNotFoundWithId(restaurant, "Не выбран ресторан");
        if (!isOpenVote()) {
            throw new CloseVoteException("Голосование закрыто до " + Integer.toString(VOTE_BORDER_TIME) + " следующего дня!");
        }
        Optional<Vote> findVote = voteRepository.findVote(userId, restId, atStartOfDay(), atEndOfDay());
